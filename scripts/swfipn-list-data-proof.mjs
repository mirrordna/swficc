...
const routes = [
  { id: "profiles", route: "/profiles/", api: "/api/source-data/search/v1", required: ["Institutions", "Entity Name"], totalAtLeast: 590_000, recordHrefPattern: "https://www\\.swfi\\.com/v1/entities/[0-9a-f]{24}" },
  { id: "people", route: "/people/", api: "/api/source-data/search/v1", required: ["People", "Name"], totalAtLeast: 100_000, recordHrefPattern: "https://www\\.swfi\\.com/v1/people/[0-9a-f]{24}" },
  { id: "transactions", route: "/transactions/", api: "/api/transactions/v1", required: ["Transactions", "Buyer Entity"], totalAtLeast: 180_000, recordHrefPattern: "https://www\\.swfi\\.com/v1/transactions/[0-9a-f]{24}" },
  { id: "deals", route: "/deals/", api: "/api/transactions/v1", required: ["Deals", "Buyer Entity"], totalAtLeast: 180_000, recordHrefPattern: "https://www\\.swfi\\.com/v1/transactions/[0-9a-f]{24}" },
  { id: "mandates", route: "/mandates/", api: "/api/live-opportunities/v1", required: ["RFPs", "Title"], totalAtLeast: 30, recordHrefPattern: "https://www\\.swfi\\.com/v1/compass/[0-9a-f]{24}" },
...
async function inspectRoute(context, spec) {
  ...
  try {
    ...
    const snapshot = await page.evaluate((recordHrefPattern) => {
      ...
      const links = [...document.querySelectorAll('a[data-source-state="on-file"], a[data-record-link="true"], table a[href], .grid a[href]')]
        .map((anchor) => ({ text: (anchor.textContent || "").trim().replace(/\s+/g, " ").slice(0, 80), href: anchor.href }))
        .filter((anchor) => recordPattern.test(anchor.href))
        .slice(0, 10);
      return { bodyText, showing, links };
    }, spec.recordHrefPattern);
